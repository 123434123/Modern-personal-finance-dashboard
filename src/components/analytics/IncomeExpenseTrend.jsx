...
function TrendTooltip({ active, payload, label, money }) {
...
export function IncomeExpenseTrend({ transactions, months }) {
    ...
    <ResponsiveContainer width="100%" height={280}>
      <AreaChart data={data} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
        ...
        <YAxis
          ...
          width={40}
        />
        <Tooltip content={<TrendTooltip money={money} />} cursor={{ stroke: 'var(--color-border-strong)' }} />
        <Legend iconType="circle" wrapperStyle={{ fontSize: 12, color: 'var(--color-text-muted)', paddingTop: 12 }} />
        <Area type="monotone" dataKey="income" name="Income" stroke="var(--chart-3)" strokeWidth={2.5} fill="url(#analyticsIncomeFill)" />
        <Area type="monotone" dataKey="expenses" name="Expenses" stroke="var(--chart-5)" strokeWidth={2.5} fill="url(#analyticsExpenseFill)" />
      </AreaChart>
    </ResponsiveContainer>
  );
}
